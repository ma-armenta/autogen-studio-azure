...
const Header = ({ meta, link }: any) => {
  ...
  const DropdownMenu = () => {
    return (
      <appContext.Consumer>
        {(context: any) => {
          // onst longestGroup = context.user?.groups?.reduce((a, b) => (a.length > b.length ? a : b), "");
          return (
            <div className="flex items-center group-dropdown" style={{width: `${100}`}}>
              <span className="mr-2">Active Group:</span>
              <div className="top-16 w-60" style={{width: `${175}px`}}>
                <Listbox value={context.activeGroup} onChange={context.setActiveGroup}>
                  <div className="relative mt-1">
                    <Listbox.Button className="relative w-full cursor-default rounded-lg bg-white py-2 pl-3 pr-10 text-left shadow-md focus:outline-none focus-visible:border-indigo-500 focus-visible:ring-2 focus-visible:ring-white/75 focus-visible:ring-offset-2 focus-visible:ring-offset-orange-300 sm:text-sm">
                      <span className="block truncate">{context.activeGroup}</span>
                    ...
                    >
                      <Listbox.Options className="absolute mt-1 max-h-60 w-full overflow-auto rounded-md bg-white py-1 text-base shadow-lg ring-1 ring-black/5 focus:outline-none sm:text-sm">
                        {context.user?.groups?.map((group: string) => (
                          <Listbox.Option
                            key={group}
                            className={({ active }) =>
